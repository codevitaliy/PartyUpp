 ...
 * Use the {@link TimedChallengeFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class TimerChallengeFragment extends ChallengeFragment {
    ...
    public TimerChallengeFragment() {
        // Required empty public constructor
    }

    public static TimerChallengeFragment newInstance(Challenge challenge, ArrayList<String> playerList) {
        TimerChallengeFragment fragment = new TimerChallengeFragment();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        SpannableString parsedDescription;

        if(Locale.getDefault().getLanguage().equals(LANG_CODE_ES)){
            parsedText = parseChallengeText(challenge.data.get("text_es"), playerList);
            parsedDescription = parseChallengeText(challenge.data.get("description_es"), playerList);
        } else {
            parsedText = parseChallengeText(challenge.data.get("text_en"), playerList);
            parsedDescription = parseChallengeText(challenge.data.get("description_en"), playerList);
        }
        ...
        btNext.setOnClickListener(v -> {
            ...
            for(ChallengeFragment.ChallengeFragmentListener listener : listeners) {
                System.out.println(challenge.data.get("duration"));
                listener.nextChallenge(Integer.parseInt(challenge.data.get("duration")), parsedDescription);
            }
        });
    }
